...
export default function StockPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const apiRows = Array.isArray(stockRes.data)
        ? stockRes.data
        : [];
  ...
  // GRIEVANCE HANDLER
  // --------------------------------------------------
  const handleGrievance = async (data: any) => {
    if (!selectedRow) return;
    try {
      const { createAdjustment } = await import("@/apiCalls");
      await createAdjustment({
        product_id: selectedRow.product_id,
        shop_id: selectedRow.shop_id,
        ...data
      });
      toast.success("Issue reported and stock adjusted");
      setModalType(null);
      fetchData();
    } catch (err: any) {
  ...
  return (
    <DashboardLayout>
      ...
      <AnimatePresence>
        ...
        {/* MODAL: REPORT GRIEVANCE (BROKEN/LOST) */}
        {modalType === "grievance" && selectedRow && (
          <GrievanceModal
            row={selectedRow}
            onClose={() => setModalType(null)}
...
function GrievanceModal({
  ...
  const amountLost = qty * row.costPrice;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (qty <= 0) return toast.error("Quantity must be greater than 0");
    if (qty > row.currentStock) return toast.error("Quantity exceeds current stock");
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 leading-tight">Report Issue</h2>
            <p className="text-sm text-slate-500 font-medium">{row.productName}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                value={qty}
                onChange={(e) => setQty(Number(e.target.value))}
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 focus:outline-none focus:ring-4 focus:ring-rose-50 focus:border-rose-200 transition-all font-bold text-slate-700"
                required
              />
            </div>
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 focus:outline-none focus:ring-4 focus:ring-rose-50 focus:border-rose-200 transition-all font-bold text-slate-700 appearance-none"
              >
          ...
          <div className="p-4 bg-rose-50/50 rounded-2xl border border-rose-100/50">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Financial Loss</span>
              <span className="text-lg font-black text-rose-600">₦{amountLost.toLocaleString()}</span>
            </div>
            <p className="text-[9px] text-rose-400 font-bold mt-1 uppercase tracking-tighter italic">Based on cost price: ₦{row.costPrice.toLocaleString()}/unit</p>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 focus:outline-none focus:ring-4 focus:ring-rose-50 focus:border-rose-200 transition-all font-medium text-slate-600 min-h-[100px] resize-none"
            />
          </div>
          ...
          <div className="flex gap-4 pt-4">
            <button
              ...
              onClick={onClose}
              disabled={submitting}
              className="flex-1 py-4 bg-slate-50 text-slate-500 rounded-2xl font-bold hover:bg-slate-100 transition-all active:scale-95 disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-bold hover:bg-rose-700 shadow-xl shadow-rose-100 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
            >
