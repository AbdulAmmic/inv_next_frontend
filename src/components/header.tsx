...
export default function Header({ onMenuClick, showMenuButton = false }: HeaderProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-lg border-b border-gray-100 px-4 md:px-6 py-3 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {/* Mobile Menu Button - Controlled via props if provided, else internal state (legacy support) */}
            {showMenuButton ? (
              <button
                onClick={onMenuClick}
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${pathname === item.path
            ...
            {shops.length > 0 && (
              <div className="hidden md:block relative min-w-[140px]">
                ...
                <select
                  ...
                  className={`w-full pl-8 pr-3 py-2 text-sm border rounded-lg appearance-none truncate ${userRole !== "admin"
      ...
      {/* ---------------- LEGACY MOBILE MENU (For pages not using DashboardLayout) ---------------- */}
      {/* Only show if we are NOT using the controlled "showMenuButton" prop mode (or if explicit fallback needed) */}
      {/* Actually, existing pages don't pass showMenuButton, so it defaults to false. */}
      {/* But they DO need a button to toggle this. I removed the internal button too. */}

      {/* Re-adding the internal hamburger button for legacy pages */}
      {!showMenuButton && (
        <div className="md:hidden fixed top-3 left-4 z-[60]">
          {/* We only render this if the Header is rendered but NO props passed. 
                 BUT: The Header component is sticky. The button needs to be INSIDE the header div.
                 Let's go back and fix the button inside the header JSX.
             */}
        </div>
      )}

      {/* Mobile Menu Overlay - Legacy */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setMobileMenuOpen(false)} />
      )}

      {/* Mobile Menu Sidebar - Legacy */}
      <div
        ref={mobileMenuRef}
        className={`fixed top-0 left-0 h-full w-64 bg-white shadow-xl z-50 transform transition-transform duration-300 ease-in-out md:hidden ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-gray-900">Tuhanas</div>
          ...
          <div className="p-4 border-b border-gray-100">
            ...
            <div className="relative">
              ...
              <select
                ...
                disabled={userRole !== "admin"}
                className={`w-full pl-10 pr-4 py-2.5 text-sm border rounded-lg ${userRole !== "admin"
        ...
        <nav className="p-2">
          {navItems.map((item) => (
            <button
              ...
              className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium mb-1 ${pathname === item.path
                ? "bg-blue-50 text-blue-600"
