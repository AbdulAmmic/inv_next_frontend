  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4">
      ...
      <div className="md:hidden flex flex-col divide-y divide-gray-100">
        {filteredProducts.map((product) => (
          <div key={product.id} className="p-4 flex flex-col gap-3 hover:bg-gray-50">
            ...
            <div className="flex items-center justify-between mt-1">
              <span className={`px-2 py-0.5 inline-flex text-[10px] leading-4 font-semibold rounded-full ${product.stock > 10 ? 'bg-green-100 text-green-800' : product.stock > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                {product.stock} in stock
              </span>
            ...
            <div className="mt-2 pt-2 border-t border-gray-100 flex justify-end">
              {isProductInCart(product.id) ? (
                <button
                  onClick={() => onRemoveFromCart(product.id)}
                  className="w-full text-red-600 hover:text-red-900 bg-red-50 px-4 py-2 rounded-lg text-sm font-bold transition-colors"
                >
                  Remove
                </button>
              ) : (
                <button
                  onClick={() => onAddToCart(product)}
                  disabled={product.stock === 0}
                  className={`w-full px-4 py-2 rounded-lg text-sm font-bold transition-colors ${product.stock === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm'}`}
                >
